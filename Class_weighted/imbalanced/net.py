...
def train(net, trainloader, valloader, epochs, learning_rate, device):
    ...
    criterion = torch.nn.CrossEntropyLoss(weight=class_weights).to(device) # add to device
...
"""
def global_test(net, testloader, class_weights, device):
    net.to(device)
    criterion = torch.nn.CrossEntropyLoss(weight=class_weights).to(device) # add to device
    all_labels, all_preds = [], []
    correct, loss = 0, 0.0
...
def test(net, testloader, class_weights, device):
    net.to(device)
    criterion = torch.nn.CrossEntropyLoss(weight=class_weights).to(device) # add to device
    all_labels, all_preds = [], []
    correct, loss = 0, 0.0
